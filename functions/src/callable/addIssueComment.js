...
export const addIssueComment = onCall(async (request) => {
  try {
    ...
    // Log audit event
    await logAuditEvent({
      action: 'equipment_issue.comment_added',
      actorId: callerUid,
      ...
      metadata: {
        ...
        textPreview: commentData.text.slice(0, 60),
      },
    });
    ...
    console.error('addIssueComment error:', err);
    await logFunctionError({
      functionName: 'addIssueComment',
      error: err,
      request,
    });
    if (err instanceof HttpsError) throw err;
    throw new HttpsError('internal', `Ошибка добавления комментария: ${err.message}`);
  }
});
