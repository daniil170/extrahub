...
export const updateIssueStatus = onCall(async (request) => {
  try {
    ...
    // Log audit event
    await logAuditEvent({
      action: 'equipment_issue.status_changed',
      actorId: callerUid,
      actorRole: callerRole,
      actorName: userData.fullName || 'Пользователь',
      targetId: issueId,
      targetType: 'equipment_issue',
      metadata: {
        newStatus: status,
        previousStatus: issueData.status,
        resolutionComment: updates.resolutionComment || null,
    ...
    await logFunctionError({
      functionName: 'updateIssueStatus',
      error: err,
      request,
    });
    if (err instanceof HttpsError) throw err;
    throw new HttpsError('internal', `Ошибка обновления статуса заявки: ${err.message}`);
  }
});
