...
export const createEquipmentIssue = onCall(async (request) => {
  try {
    ...
    // Log audit event
    await logAuditEvent({
      action: 'equipment_issue.created',
      actorId: callerUid,
      actorRole: callerRole,
      actorName: userData.fullName || 'Преподаватель',
      targetId: issueRef.id,
      targetType: 'equipment_issue',
      metadata: {
    ...
    console.error('createEquipmentIssue error:', err);
    await logFunctionError({
      functionName: 'createEquipmentIssue',
      error: err,
      request,
    });
    if (err instanceof HttpsError) throw err;
    throw new HttpsError('internal', `Ошибка создания заявки на ремонт: ${err.message}`);
  }
});
